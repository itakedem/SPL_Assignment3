...
public class EncoderDecoder implements MessageEncoderDecoder<Message>
{
    ...
    {
        pushByte(nextByte);
        if(len == 2)        //means we can extract opcode
    ...
    private Message lowOpDecode(byte nextByte)
    {
        ...
        else if (nextByte == '\0' & counter == 1)       //means we finished reading from socket
        {
    ...
    private Message highOpDecode(byte nextByte)
    {
        if (opCode == 4 | opCode == 11)
            return new Message(opCode);     //logout or mycourses
    ...
    {
        byte[] opcode = shortToBytes((short)message.getOp_code());      //the opcode of the return message
        byte[] Mopcode = shortToBytes((short)message.getMessage_op_code());     //opcode of the original message
        bytes = combiner(opcode,Mopcode);
        if (message.getOp_code() == 13)
            return bytes;
        bytes = combiner(bytes,message.getOutput().getBytes(StandardCharsets.UTF_8));       //adds the optional
        byte[] b = {'\0'};
